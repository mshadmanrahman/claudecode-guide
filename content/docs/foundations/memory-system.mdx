...
<Callout type="warn" title="Memory is not automatic">
Memory files are **not** read automatically at session start. You need to either reference them in your `CLAUDE.md` (e.g., "Read `memory/MEMORY.md` at startup") or use a startup hook. Without explicit configuration, Claude Code will not know your memory files exist.
</Callout>

<DemoCard title="Memory in action (with CLAUDE.md configured)" steps={[
  { type: 'cmd', text: 'claude' },
  { type: 'out', text: 'Reading CLAUDE.md... following instruction to load MEMORY.md' },
...
## Wait — Memory vs CLAUDE.md vs Config. What's the Difference?

This confuses everyone. Here's the simple version:

| File | What it does | Analogy |
|------|-------------|---------|
| **CLAUDE.md** | Tells Claude about *this project* — tech stack, build commands, coding rules | A project README for Claude |
| **Memory files** | Tells Claude about *you* — your role, preferences, corrections, active work | Your personal profile that follows you |
| **Config files** (`.claude/settings.json`) | Technical settings — which tools are allowed, model preferences, permissions | App settings (like dark mode on/off) |

**CLAUDE.md** lives in your project folder and changes per project. **Memory** lives in `~/.claude/` and persists across all projects and sessions. **Config** is plumbing you rarely touch.

Think of it this way: CLAUDE.md is project context ("this is a Next.js app"), memory is personal context ("I'm a PM who hates verbose output"), and config is machine settings ("allow the Read tool without asking").
...
`MEMORY.md` acts as a table of contents. When you configure Claude Code to read it (via CLAUDE.md or a startup hook), it serves as an index so Claude knows which memory files exist. You control which files get loaded — Claude reads what you tell it to read, not what it decides is relevant on its own.

## The /memory Command

Claude Code has a built-in `/memory` command that provides native memory persistence without any file setup. When you run `/memory` in a session, Claude Code saves key facts, preferences, and corrections to its internal memory store. These persist across sessions automatically.

This is the simplest way to get started with memory — no files to create, no CLAUDE.md configuration needed. Just tell Claude something important and run `/memory` to save it.

The file-based memory system described in this guide is a **complementary approach** that gives you more control: you can organize memories by category, version them in Git, share them across machines, and build structured indexes. Many users combine both — `/memory` for quick personal preferences, file-based memory for project and team context.
...
- [ ] Create a `MEMORY.md` file in your `memory/` directory (e.g., `memory/MEMORY.md`)
