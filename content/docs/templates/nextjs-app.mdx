...
description: "Copy-paste CLAUDE.md for Next.js App Router projects. Server components by default, async params, testing conventions baked in."
---
...
Use this template for standalone Next.js applications using the App Router with a `src/` directory structure. I built this against a real production app and it handles the conventions that trip people up the most: server component defaults, the async API patterns added in Next.js 15, and a testing setup that keeps tests next to the code they cover.
...
3. Adjust the directory layout section if your project doesn't use a `src/` directory
4. Add project-specific rules as you find patterns Claude keeps getting wrong
