...
type Journey = "recruiting" | "people-ops" | "communication";
...
const JOURNEY_DATA: Array<{
...
}> = [
  {
    ...
    note: "Give Claude the role context. It writes the job description, interview questions, and scoring rubric.",
    clusters: [
      {
        cluster: "Recruiting",
        guideKeys: [
  ...
  },
  {
    ...
    note: "Onboarding plans from a hiring brief, review templates from your notes, and exit interview analysis in minutes.",
    clusters: [
      {
        cluster: "People ops",
        guideKeys: [
  ...
  },
  {
    ...
    note: "Policy documents people will read, and employee communications that land the right way.",
    clusters: [
      {
...
const TAB_ACTIVE_BORDER: Record<Journey, string> = {
  recruiting: "border-blue-600",
  "people-ops": "border-emerald-600",
  communication: "border-violet-600",
};

const ENV_BADGE_STYLES: Record<Journey, string> = {
  recruiting: "bg-blue-50 text-blue-800 dark:bg-blue-900/30 dark:text-blue-300",
  "people-ops":
...
function DifficultyBadge({ level }: { level: "beginner" | "intermediate" }) {
  ...
  return (
    <span
      className={`rounded-full px-2 py-0.5 text-[10px] font-medium ${styles}`}
    >
      {level}
    </span>
  );
...
export function HrGuideCards() {
  const [ref, inView] = useInView(0.05);
  const [journey, setJourney] = useState<Journey>("recruiting");
      ...
      <div className="mx-auto max-w-5xl px-6">
        ...
        >
          ...
          <p className="mt-4 max-w-lg text-fd-muted-foreground">
            Three areas where Claude saves the most HR time. Select one to see
            the{" "}
            <span className="font-medium text-fd-foreground">
              {journeyGuideCount} guides
            </span>{" "}
            for that area.
          </p>
        </div>
          ...
          <div className="flex gap-1 border-b border-fd-border">
            {JOURNEY_DATA.map((j) => (
              <button
                ...
                onClick={() => {
                  setJourney(j.id);
                  trackEvent("hr_journey_tab_click", {
                    journey: j.id,
                    section: "for-hr",
                  });
                }}
                ...
                <div className="grid gap-4 sm:grid-cols-2">
                  {clusterGuides.map((guide) => {
                    ...
                    return (
                      <Link
                        key={guide.slug}
                        href={`/for-hr/${guide.slug}`}
                        onClick={() =>
                          trackEvent("hr_guide_card_click", {
                        ...
                        style={{
                          animationDelay: `${(ci * 3 + (num % 3)) * 80 + 100}ms`,
                        }}
                      >
