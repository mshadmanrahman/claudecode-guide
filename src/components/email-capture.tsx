...
export function EmailCapture() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Open Substack subscription in a new tab with pre-filled email
    // This is the simplest integration that works without an API key
    window.open(
      `https://shadmanrahman.substack.com/subscribe?email=${encodeURIComponent(email)}`,
      '_blank'
    );
  ...
  if (status === 'success') {
    return (
      <div className="rounded-xl border border-green-500/20 bg-green-500/5 p-6 text-center">
        ...
        <p className="font-medium text-fd-foreground">Check the tab that just opened!</p>
        <p className="mt-1 text-sm text-fd-muted-foreground">
          Confirm your subscription on Substack to get updates.
        </p>
        <button
          type="button"
