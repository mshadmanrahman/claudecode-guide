...
export async function generateMetadata({
  ...
  const guide = MICROSOFT_GUIDES[slug];

  if (!guide) return {};

  const canonicalUrl = `https://claudecodeguide.dev/for-microsoft/${slug}`;

  return {
    title: { absolute: `${guide.title} | Claude for Microsoft Office` },
...
export async function generateStaticParams() {
  return Object.keys(MICROSOFT_GUIDES).map((slug) => ({ slug }));
}
...
export default async function MicrosoftGuidePage({
  params,
}: {
  params: Promise<{ slug: string }>;
}) {
  ...
  return (
    <div className="flex flex-col bg-fd-background">
      <ArticleSchema
        ...
        description={guide.description}
        url={`https://claudecodeguide.dev/for-microsoft/${slug}`}
      />
      <PersonaGuideTracker slug={guide.slug} title={guide.title} section="for-microsoft" />
      <article className="mx-auto w-full max-w-3xl px-6 pt-12 pb-24">
        ...
        <header className="mb-12">
          ...
          <p className="mt-4 text-lg text-fd-muted-foreground">{guide.description}</p>
        </header>
        ...
        {guide.outcomes ? (
            ...
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
              {guide.outcomes.map((outcome, i) => (
                <div key={i} className="rounded-xl border border-fd-border bg-fd-card p-5">
                  ...
                  <p className="mt-3 text-sm text-fd-foreground leading-relaxed">{outcome}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          <div data-persona-guide-intro className="mb-12 rounded-xl border border-fd-border bg-fd-card p-6">
            <p className="text-sm leading-relaxed text-fd-muted-foreground">{guide.intro}</p>
          </div>
        )}
        ...
        {guide.promptContrast && (
          <div className="mb-12 space-y-3">
            ...
            {guide.promptContrast.why && (
              <p className="px-1 text-sm text-fd-muted-foreground leading-relaxed italic">
                {guide.promptContrast.why}
              </p>
            )}
          </div>
          ...
          {guide.steps.map((step, index) => (
            <section key={index}>
              <div className="flex items-start gap-4 mb-4">
                ...
                <div>
                  <h2 className="text-lg font-medium text-fd-foreground">{step.title}</h2>
              ...
              {step.code && (
                <div className="ml-12 mt-4">
                  <CopyBlock code={step.code.snippet} language={step.code.language} />
                </div>
              )}
