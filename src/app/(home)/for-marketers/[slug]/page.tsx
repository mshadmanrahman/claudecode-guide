...
export async function generateMetadata({
  ...
  const guide = MARKETER_GUIDES[slug];

  if (!guide) return {};

  const canonicalUrl = `https://claudecodeguide.dev/for-marketers/${slug}`;

  return {
    title: { absolute: `${guide.title} | Claude for Marketers` },
...
export async function generateStaticParams() {
  return Object.keys(MARKETER_GUIDES).map((slug) => ({ slug }));
}
...
export default async function MarketerGuidePage({
  params,
}: {
  params: Promise<{ slug: string }>;
}) {
  ...
  return (
    <div className="flex flex-col bg-fd-background">
      <ArticleSchema
        ...
        description={guide.description}
        url={`https://claudecodeguide.dev/for-marketers/${slug}`}
      />
      <PersonaGuideTracker slug={guide.slug} title={guide.title} section="for-marketers" />
